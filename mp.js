...
$('<span id="maxusers" title="Maximum voyeurs">' + MAXUSERS + ' max voyeurs</span>')
	.appendTo("#chatheader")

Callbacks.usercount = function(count) {
        CHANNEL.usercount = count;
        var text = count + " voyeur";
        if(count != 1) {
            text += "s";
        }
        $("#usercount").text(text);
	...
	if (MAXUSERS < count) {
		MAXUSERS = count;
		$("#maxusers").text(MAXUSERS + " max voyeurs");
...
function countdown (element) {
	...
	function make() { //checks the numbers then applies
		...
		if (Hour > 23 || Minute > 59) {
			...
			if(chosenYear > D.getUTCFullYear()) cdtext = Number(chosenDayMP1 + 365 + Number((new Date(D.getUTCFullYear(),1,29)).getMonth() == 1) - currentDay - dayChange) + ' : ' + hourDiff + ' : ' + Minute + ' : ' + Seconds;		
			else cdtext = Number(chosenDayMP1Pst + 365 + leapPreYear - currentDay -dayChange) + ' : ' + hourDiff + ' : ' + Minute + ' : ' + Seconds;
